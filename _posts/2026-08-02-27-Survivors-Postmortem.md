...
This is the postmortem. It is mostly about the things that didn't ship.

I think that's the honest way to write one of these. Every finished game is a small pile of things that worked sitting on top of a very large pile of things that didn't, and the second pile is where all the interesting engineering lives. The features that shipped are visible in the game; you can go look at them. The features that died are only visible in the git history, and they're the ones that actually taught me something.
...
I've wanted to make games since I was a kid. That's the easy answer, it's true, and it explains nothing about why this particular game got made in this particular four months.

The real answer is that I was laid off from Meta.

If you've been through one, you know the specific texture of it. It isn't grief exactly, and it isn't anger exactly. It's the floor moving. Some distant authority you will never meet, in a room you'll never see, makes a decision that rearranges your entire life — and the decision has nothing to do with you, or your work, or whether you were good at it. It's about a number going up somewhere. You are a line item in a story about someone else getting slightly richer. And there is nothing whatsoever you can do about it.
...
> _Amnesia: The Dark Descent_ crossed with _Clue_, set in the cold silence of deep space.

You are aboard the *Orpheus*, a generation ship carrying colonists to a habitable star. Everyone is asleep. Something has gone very wrong, and the ship's AI has been waking crew members one at a time to deal with it. They have all died. You are next.
...
In mid-April I decided it should be able to actually talk to you. Not menu topics — a real conversation with a language model running locally, so the AI could improvise, so a second playthrough wouldn't hand you identical text, so the thing could feel like a mind instead of a lookup table. I wired in a `DialogProvider` interface with two implementations behind it: a static one that read authored text, and one that talked to a locally hosted model. Two commits later — "Added local LLM" and "LLM improvements" — it worked.
...
So I cut it and hand-authored everything instead. The `DialogProvider` abstraction survived — the static provider is still what ships — and the console eventually grew into a genuinely nice system: a random-access records browser you interrogate like a wiki, plus a Morrowind-style conversational mode where the AI picks the topic instead of you. Every line has three variants depending on whether the AI is neutral, compromised, or the antagonist itself.

Here's the thing I didn't expect: hand-authoring made the game *better*, not just cheaper. In a mystery, every line the AI says is potential evidence. A model that improvises can accidentally contradict the truth the run rolled at startup, and in a whodunnit, an accidental contradiction isn't flavor — it's a lie the player will build a theory on and then be punished for. Authored text can be audited. Improvised text can't. I went in chasing replayability and came out understanding that in this specific genre, determinism *is* the feature.

## The Voice

The ship's AI doesn't just print text at you — it speaks. Not recorded voice acting, but [DECtalk](https://en.wikipedia.org/wiki/DECtalk), a formant synthesizer running as a native extension and generating audio at runtime, so the AI can say your current survivor's name, the deck the fault just occurred on, and how many people are left.

DECtalk is the family of synthesizer behind the most recognizable computer voice of the last forty years, and that was the whole point. The first version I tuned was a deliberate nod to Stephen Hawking. I spent a long evening pushing pitch, rate, and formant parameters around, trying to land as close to that voice as I could get, and I got reasonably close.

Then the photographs of Hawking on Epstein's island surfaced, and I no longer wanted the reference in my game. It wasn't a difficult call — it took about a minute. The homage was the only reason the tuning existed, and once the homage had soured, the tuning had no reason to exist either.
...
Five clauses of tangled predicate logic became a one-line mask toggle. And the correctness argument got much stronger, because the engine enforces it in C++ at sub-step resolution. Software gates have edge cases: intermediate waypoints, single-tick prediction, directional gradients in the avoidance solver. Physics doesn't have edge cases. Physics is the edge case.

The general lesson, which I have now relearned in three different domains: when a rule keeps leaking, stop adding checks and go find a lower layer that can enforce it for you. If you can express your invariant in terms of something the engine already guarantees, do that instead of guarding it yourself.
...
Deleting a feature is a *refactor*, and it is exactly as invasive as adding one. I had that reflex — "cutting scope will save time" — and it's true in the aggregate and completely false in the moment. That cut cost me most of a working day I did not have, six days from a hard external deadline.
...
**Randomized survivor stats.** The original design had each new survivor roll random values for health, endurance, speed, strength, and aim. It's the obvious roguelike move and it was wrong for this game. If the person you become is a stat block, the Human Counter is just a life bar, and I'd written a whole design document about how it isn't. So I replaced the roll with a hand-authored roster of 27 named people — Mira Okafor, chief mechanic, propulsion and coolant-loop certified, eight years in the Lagos shipyard — each with their own stats, height, and bio, awakened in manifest order. There's a companion document listing the 73 who died before you woke up, feeding the dossiers you find on their corpses. 27 + 73 = 100. That's the whole point.
...
**Quest VR.** Listed as a target platform for months on the strength of pure optimism. When I finally audited it, the project had exactly zero XR scaffolding — no origin node, no controller nodes, no OpenXR plugin — and, more damningly, the entire post-processing chain that *makes* the game look like the game is screen-space and renders to one eye. Retrofitting it for stereo, plus world-space HUD, comfort options, and an action map, is a multi-month project on its own. Demoted to "future target," honestly this time.
...
_The colony mission patch. One of the game's real clues is a burned, defaced copy of it — a prop that sat blocked in the art queue until the patch itself existed. Orpheus went into the underworld to bring someone back and looked over his shoulder at the worst possible moment, which felt about right for a colony ship._

## What I'd Do Differently

**Cut earlier.** Not "cut more" — the cuts I made were correct. But the Jefferies tubes were visibly at risk for at least a week before I killed them, and every day I waited made the deletion more expensive, because more systems had grown references to a thing that was going to be deleted anyway. There's a bias where holding a decision open feels like preserving optionality, and for scope cuts it's the opposite: you're accruing interest on a debt you've already privately decided to pay.
...
I also got the thing I actually came for. I started this with a knot in my chest over a decision made somewhere far above me, and I finished it with a shipped game, a store page, and a room full of people at a convention playing it — none of whom had any idea what was underneath it, and none of whom needed to. Making the helplessness into something with rules, and a shape, and an ending you can reach is a real trick, and it works on the person building it at least as well as on the person playing it. Recommended, if you've got four months.
